...
# Used to run predictions on testset for checkpoint (including output layer: sequence_output)
...
# Used to run predictions on testset for checkpoint (including more output layers (sequence_output and the dense_layer from RobertaClassificationHead))
...
# using when training (not outputting extra layer (sequence_output) to avoid debugging Trainer)
class RobertaForSequenceClassification_fromTransformersLinear(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
# Used to train/finetune roberta using original script from Transformers
class RobertaForSequenceClassification_fromTransformers(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        #Modification (output dense layer in RobertaClassificationHead): 
        self.classifier = RobertaClassificationHead_fromTransformers(config)
...
class RobertaClassificationHead_Original(nn.Module):
    ...
    def forward(self, features, **kwargs):
        ...
        x = torch.tanh(x_dense)
        x = self.dropout(x)
        x = self.out_proj(x)
        return x, x_dense

class RobertaClassificationHead_fromTransformers(nn.Module):
    """Head for sentence-level classification tasks."""
...
class RobertaClassificationHead_Linear(nn.Module):
    """Linear-head for sentence-level classification tasks."""
